...
### 需求目标
- 前后端全面对接：前端所有核心页面与后端 API 实际联通
- 系统流畅性提升：首包更快、交互更稳、长对话不掉帧
- 后端能力增强：并发、缓存、流式响应、错误治理全面优化
- 结构精简：超长模块拆分，复用逻辑抽象

### 验收指标
- 对话与群聊 SSE 流式响应无阻塞、无断流
- `/api/health`、`/api/stats` 能反映真实服务状态
- 模型/视觉/语音/检索均可被前端稳定调用
- 前端关键路径请求数下降、响应时间稳定
...
**PowerShell 用户**：可使用 `.\run_server.ps1` 快速启动 Web 服务
