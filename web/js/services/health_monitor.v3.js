...
const HealthMonitor = (function() {
    ...
    function init() {
        ...
        try {
            ...
            console.warn('[HealthMonitor] 初始化服务映射失败:', e);
        }
        console.log('[HealthMonitor v2.1] 初始化（轻量模式）');
    }

    function initializeServices() {
        console.log('[HealthMonitor] 开始初始化服务，检查 UnifiedAPIClient...');
        // 添加防御性检查，避免 UnifiedAPIClient 未定义导致的错误
        if (typeof UnifiedAPIClient === 'undefined') {
            console.warn('[HealthMonitor] ✅ UnifiedAPIClient 未加载，使用静态配置');
            // 使用默认服务配置
            state.services['api'] = {
                name: 'api',
    ...
    async function checkAllServices() {
        try {
            ...
            const url = `http://${base}:5001/api/health`;
    ...
    // 轻量化绑定：对接首页状态灯
    function startLightweightStatusBinding(intervalMs = 5000) {
        init();
        if (state.checkTimer) clearInterval(state.checkTimer);
        checkAllServices();
        state.checkTimer = setInterval(checkAllServices, intervalMs);
        console.log('[HealthMonitor] 状态灯已绑定 /api/health');
    }
